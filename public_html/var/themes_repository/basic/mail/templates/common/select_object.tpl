    ...
    <div id="select_{$selected_id}_wrap_{$suffix}" class="select-popup cm-popup-box cm-smart-position hidden">
        <ul class="cm-select-list select-list flags">
            {foreach from=$items item=item key=id}
                <li><a rel="nofollow" href="{"`$link_tpl``$id`"|fn_url}" class="{if $display_icons == true}item-link clearfix{/if} {if $selected_id == $id}active{/if} {if $suffix == "translate_box"}cm-lang-link{/if}" {if $display_icons == true}data-ca-country-code="{$item.country_code|lower}"{/if} data-ca-name="{$id}">
                    {if $display_icons == true}
