...
class Styles
{
    ...
    {
        $manifest = array();
        $manifest_path = $this->styles_path . 'manifest.json';
    ...
    private function getStyleImage($style_id)
    {
        $url = '';
        $path = $this->getStyleFile($style_id, 'png');

        if (file_exists($path)) {
            $url = Registry::get('config.current_location') . '/' . $this->getRelativePath($path);
        }
    ...
    {
        return str_replace(Registry::get('config.dir.root') . '/', '', $path);
    } 
