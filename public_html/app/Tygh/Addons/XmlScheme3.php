...
class XmlScheme3 extends XmlScheme2
{
    ...
    protected function _getTranslations($xml_node, $type = '', $parent_id = '', $value_name = 'value')
    {
        ...
        foreach (fn_get_translation_languages() as $lang_code => $_v) {
            ...
            $value = isset($po_values[$lang_code][$po_id]) ? $po_values[$lang_code][$po_id]['value'] : $xml_node->xpath("translations/item[(not(@for) or @for='name') and @lang='$lang_code']");

            if (!empty($value) && is_array($value)) {
                $value = (string) current($value);
            }

            if (!empty($value) || !empty($default_translation[$value_name])) {
                $translations[] = array(
                    'lang_code' =>  $lang_code,
