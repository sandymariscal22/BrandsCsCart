...
function fn_change_category_parent($category_id, $new_parent_id)
{
    if (!empty($category_id)) {
        ...
        $paths = db_get_hash_array("SELECT category_id, parent_id, id_path FROM ?:categories WHERE category_id IN (?n)", 'category_id', array($new_parent_id, $category_id));

        if (!empty($paths)) {
            // new category, generate path
            $id_path = '';
            if (empty($paths[$category_id]['id_path'])) {
                if (empty($new_parent_id)) {
                    $id_path = $category_id;
                } else {
                    $id_path = $paths[$new_parent_id]['id_path'] . '/' . $category_id;
                }

                db_query("UPDATE ?:categories SET id_path = ?s WHERE category_id = ?i", $id_path, $category_id);

                $paths[$category_id]['id_path'] = $id_path;
            }

            if (isset($paths[$category_id]['parent_id']) && $paths[$category_id]['parent_id'] != $new_parent_id) {
                if (!empty($paths[$new_parent_id]['id_path'])) {
                    db_query("UPDATE ?:categories SET parent_id = ?i, id_path = ?s WHERE category_id = ?i", $new_parent_id, $paths[$new_parent_id]['id_path'] . "/$category_id", $category_id);
                    db_query("UPDATE ?:categories SET id_path = CONCAT(?s, SUBSTRING(id_path, ?i)) WHERE id_path LIKE ?l",  $paths[$new_parent_id]['id_path'] . "/$category_id/", strlen($paths[$category_id]['id_path'] . '/') + 1, $paths[$category_id]['id_path'] . '/%');
                } else {
                    db_query("UPDATE ?:categories SET parent_id = ?i, id_path = ?i WHERE category_id = ?i", $new_parent_id, $category_id, $category_id);
                    db_query("UPDATE ?:categories SET id_path = CONCAT(?s, SUBSTRING(id_path, ?i)) WHERE id_path LIKE ?l", "$category_id/", strlen($paths[$category_id]['id_path'] . '/') + 1, $paths[$category_id]['id_path'] . '/%');
                }
